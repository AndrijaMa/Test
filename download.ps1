...
$Help = Write-Host "-License: Enter you tableau server key or type Trial if you want to use a 14 day trial"
...
function Write-ToLog ($text) {
    ...
    Write-Host  $message
    Write-Output $message | Out-file $folder$log_file -Append -Encoding default
...
function install(
                    ...
                    $LicenseKey
                        ...
                        try {
                                ...
                                Write-ToLog -text "Started downloading Tableau Server media" 
                                Invoke-WebRequest -Uri $url -OutFile $folder"tableau_server.exe"
                                Write-ToLog -text "Finnished downloading Tableau Server media"
                                ...
                                Write-ToLog -text  "Tableau server installation finnished"
                                ...
                                $Env:path += $tsm_path
                                #Activate license
                                Write-Host "Activating Tableau Server License"
                                if($LicenseKey -eq '')
                                {
                                    Write-Host "-LicenseKey parameter was not entered       ! Please enter your license key or type Trial to activeate a 14 day trial key."
                                }
                                ...
                                Write-ToLog -text  "Completed TM changes"
